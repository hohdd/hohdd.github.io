...
- Một số phím tắt **Shortcuts**
    ...
    - Cursor 3D:
        - Giữ *Shift + click chuột phải*{:.w3-text-red} để thay đổi vị trí Cursor 3D
        - Để reset về (0,0,0) bấm *Shift + C*{:.w3-text-red}
        - **Shift + S**{:.w3-text-red} để hiện Menu Tròn cài đặt Cursor 3D (Tip: chuyển Cursor về mặt đang chọn > chuyển Origin To Cursor)
        - Move selected Object to Cursor
    - **Screen View**{:.w3-text-orange}:
        ...
        - Viewport Shading Mode (Wireframe, Solid, Material Preview, Rendered): phím tắt "**Z**{:.w3-text-red}"
    - Selections
        - **Tab**{:.w3-text-red} để chuyển chế độ *Object-Edit*, **1-2-3**{:.w3-text-red} (*Đỉnh-Cạnh-Mặt*) để chuyển Select Mode
        ...
        - *Chọn Đỉnh-Cạnh-Mặt rộng thêm*{:.w3-yellow} bằng cách giữ **Alt + Click** thêm 1-2-3 lần nữa vào Đỉnh-Cạnh-Mặt
    - Snapping
...
### Modeling
- **Thêm một vật thể** (*Mesh, Curve, Surface, Metaball, Empty, Image, Light, Camera...*): *Shift+A*{:.w3-text-red}
...
- Cách thêm một ảnh vào Blender (*trước khi thêm ảnh cần để Viewport là Front*{:.w3-yellow}):
    ...
    - Nếu ảnh đứng chơi vơi: **Alt+G** (*move selected to Cursor*) > **Alt+R** (*reset rotate*)
- **Shift + R**{:.w3-text-red} (**R**epeat) để nhắc lại lệnh
...
- Khi **E**xtrude có thể "*match*{:.w3-yellow}" khoảng cách Extrude so với Object khác (giống trong Sketchup) bằng cách giữ Ctrl để bắt điểm ở Object khác (vì Edit Mode chỉ có hiệu lực trên Object đã được chọn trước khi chuyển sang Edit Mode từ Object Mode)
...
- Một **Object** có thể *tùy chọn hiển thị riêng kiểu Wireframe*{:.w3-yellow} (thích hợp làm Bound Reference): **Object Properties > Viewport Display > Display As > Wire** > thêm việc tắt Selectable để làm Reference để không thể Select.
- **Để san bằng**{:.w3-yellow} *theo một trục* (Y): **S**{:.w3-text-red}cale *theo trục* **Y** *với* **giá trị = 0**{:.w3-text-red}. *Áp dụng cho cả phía mặt ngoài và đường lưới trong mặt gấp khúc*.
