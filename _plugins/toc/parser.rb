        ...
        def inject_anchors_into_html
          @entries.each do |entry|
            entry[:header_content].add_previous_sibling(
              %(<a class="anchor" href="##{entry[:id]}" aria-hidden="true" spy-title="#{entry[:text]}"><span class="octicon octicon-link"></span></a>)
            )
          end
        ...
        def build_toc_list(entries)
          ...
          while i < entries.count
            ...
            if entry[:h_num] == min_h_num
              ...
              toc_list << %(<li class="#{@configuration.item_class} #{@configuration.item_prefix}#{entry[:node_name]}"><a href="##{entry[:id]}" spy-toc="#{entry[:text]}">#{entry[:text]}</a>)
