(function () {
    ...
    function put(data, isAppend) {
        if (isObject(data)) {
            return addObject(data)
        }
        if (isArray(data)) {
            return addArray(data, isAppend)
        }
        return undefined
    }
    function clear() {
    ...
    function addArray(_data, isAppend) {
        const added = []
        if (!isAppend) {
            clear()
        }
    ...
    (function (window) {
      ...
      /* removed: const _$utils_9 = require('./utils') */;
        ...
        window.DongHDSearch = function (_options) {
            ...
            if (isArray(options.json)) {
                for (let index = 0; index < options.json.length; index++) {
                    const url = options.json[index];
                    // initWithURL(url)
        ...
        function throwError(message) {
            throw new Error('DongHDSearch --- ' + message)
        }
    })(window)
